...
class ActuatorControl(Node):
    ...
    def publish_actuator_data(self):
        ...
        if self.in_turning_phase:
            if time.time() - self.turn_start_time < self.turning_time:
                # Still within committed turn time — keep turning
                msg = Actuator()
                msg.angle = self.last_turn_angle
                msg.rpm = self.last_turn_rpm
        ...
        if safety_state == "safe":
            ...
            if front >= self.safe_threshold + 20:
                ...
                if right < self.safe_threshold + 10:
                    self.start_turn(self.turn_left_angle, self.high_speed)
                elif left < self.safe_threshold + 10:
                    self.start_turn(self.turn_right_angle, self.high_speed)
                else:
                    msg.rpm = self.high_speed
                    msg.angle = self.forward_angle
        ...
        # SAFETY STATE = "BARELY_SAFE"
            ...
            if front < self.safe_threshold:
                ...
                if right < self.safe_threshold + 10 and left < self.safe_threshold + 10:
                    if right < left:
                        self.start_turn(self.turn_left_angle, self.slow_speed)
                    else:
                        self.start_turn(self.turn_right_angle, self.slow_speed)
    ...
    def start_turn(self, angle, rpm):
        self.in_turning_phase = True
        self.turn_start_time = time.time()
        self.last_turn_angle = angle
        self.last_turn_rpm = rpm
